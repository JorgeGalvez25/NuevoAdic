...
namespace EstandarCliente.Infrastructure.Module
{
    public class ServiciosMenuAplicacion : IServiciosMenuAplicacion
    {
        ...
        {
            string RutaImagenCo = System.Configuration.ConfigurationSettings.AppSettings["RutaImagenCo"];
            ...
            if (icono == null)
            {
                var tipo = typeof(EstandarCliente.Infrastructure.Module.Properties.Resources);
                foreach (var item in tipo.GetProperties(System.Reflection.BindingFlags.Static | System.Reflection.BindingFlags.NonPublic))
                {
                    if (item.Name.Equals(imagen))
                    {
                        icono = (System.Drawing.Image)item.GetValue(tipo, null);
                        break;
                    }
                }
            }
