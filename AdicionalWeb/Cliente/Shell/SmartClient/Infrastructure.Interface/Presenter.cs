    ...
    public abstract class Presenter<TView> : IDisposable
    {
        ...
        protected virtual void CloseView()
        {
            try
            {
                ...
                if (wks != null)
                {
                    System.Windows.Forms.UserControl uc = (System.Windows.Forms.UserControl)wks.SmartParts[0];
                    uc.Enabled = true;
                    wks.Close(View);
        ...
        public virtual void CloseView(string Invoker)
        {
            Services.IWorkspaceLocatorService locator = WorkItem.Services.Get<Services.IWorkspaceLocatorService>();
            IWorkspace wks = locator.FindContainingWorkspace(WorkItem, View);
            ...
            if (wks != null)
            {
                ...
                {
                    uc = (System.Windows.Forms.UserControl)wks.SmartParts[0];
                    uc.Visible = false;
                }
                try { wks.Close(View); }
                catch { }
            ...
            if (Invoker != "" || Invoker != null)
            {
                foreach (object o in wks.SmartParts)
                {
                    string vname = o.ToString();

                    if (vname.Contains(Invoker))
                    {
                        wks.Activate(o);
                        obj = o;
                        //EncontroSmartPart = true;
                        break;
                    }
                }
            }
