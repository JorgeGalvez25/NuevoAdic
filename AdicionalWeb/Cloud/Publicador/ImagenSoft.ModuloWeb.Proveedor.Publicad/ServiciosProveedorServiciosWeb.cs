...
{
    public class ServiciosProveedorServiciosWeb : IServiciosWebProveedor, IServiciosWebPerform, IDisposable
    {
        #region Propiedades
        ...
        private ProxyServiciosWeb servicioMonitor;
        private ProxyServiciosWebPerform ServicioPerform;
        ...
        public ServiciosProveedorServiciosWeb(Sesion sesion, TipoConexionUsuario tipo)
        {
            ...
            this.Sesion = sesion;
            this.servicioMonitor = new ProxyServiciosWeb(serviciosConexion.GetMonitorBinding(), new EndpointAddress(serviciosConexion.HostMonitor.ToString()));
            this.ServicioPerform = new ProxyServiciosWebPerform(serviciosConexion.GetPerformBinding(), new EndpointAddress(serviciosConexion.HostPerform.ToString()));
        }
        ...
        private T GetFunctionMonitor<T>(Sesion sesion, SolicitudServiciosWeb solicitud)
        {
            ...
            try
            {
                if (servicioMonitor.State == CommunicationState.Closed)
                {
                    this.servicioMonitor = new ProxyServiciosWeb(serviciosConexion.GetMonitorBinding(), new EndpointAddress(serviciosConexion.HostMonitor.ToString()));
                }
                servicio = this.servicioMonitor.GetChannel;
                ...
                if (servicio != null)
                {
                    using (IClientChannel clientInstance = ((IClientChannel)servicio))
                    {
                        if (clientInstance.State == System.ServiceModel.CommunicationState.Faulted)
                        {
                            ...
                            finally { this.servicioMonitor = new ProxyServiciosWeb(serviciosConexion.GetMonitorBinding(), new EndpointAddress(serviciosConexion.HostMonitor.ToString())); }
                        }
                    }
                }
            }
        ...
        private void GetFunctionPerform(Action<IServiciosWebPerform> fn)
        {
            ...
            try
            {
                if (ServicioPerform.State == CommunicationState.Closed)
                {
                    this.ServicioPerform = new ProxyServiciosWebPerform(serviciosConexion.GetPerformBinding(), new EndpointAddress(serviciosConexion.HostPerform.ToString()));
                }
                ...
                if (servicio != null)
                {
                    using (IClientChannel clientInstance = ((IClientChannel)servicio))
                    {
                        if (clientInstance.State == System.ServiceModel.CommunicationState.Faulted)
                        {
                            ...
                            finally { this.ServicioPerform = new ProxyServiciosWebPerform(serviciosConexion.GetPerformBinding(), new EndpointAddress(serviciosConexion.HostPerform.ToString())); }
                        }
                    }
                }
            }
        ...
        ~ServiciosProveedorServiciosWeb()
        {
        ...
        public void Dispose()
        {
            if (this.servicioMonitor != null)
            {
                try { if (this.servicioMonitor.State == CommunicationState.Faulted) { this.servicioMonitor.Abort(); } }
                catch { }
                try { if (this.servicioMonitor.State != CommunicationState.Closed) { this.servicioMonitor.Close(); } }
                catch { }
            }

            if (this.ServicioPerform != null)
            {
                try { if (this.ServicioPerform.State == CommunicationState.Faulted) { this.ServicioPerform.Abort(); } }
                catch { }
                try { if (this.ServicioPerform.State != CommunicationState.Closed) { this.ServicioPerform.Close(); } }
                catch { }
            }
        }
