...
{
    public class ServiciosProveedorServiciosWeb : IModuloWebProveedor,
                                                  IModuloWebPerform,
                                                  IDisposable
    {
        #region Propiedades
        ...
        private T GetFunctionMonitor<T>(Sesion sesion, SolicitudHostWeb solicitud)
        {
            try
            {
                ...
                ProxyServiciosWeb servicioMonitor = new ProxyServiciosWeb(serviciosConexion.GetMonitorBinding(), new EndpointAddress(serviciosConexion.HostMonitor.ToString()));
                try
                {
                    servicio = servicioMonitor.GetChannel;

                    if (!this.Sesion.Compare(sesion)) { this.Sesion = sesion; }
                    if (solicitud.Sesion == null) { solicitud.Sesion = this.Sesion; }
                    ...
                    if (!resultado.EsValido) { throw new Exception(resultado.Mensaje); }
                    ...
                    if (servicio != null)
                    {
                        ...
                        else
                        {
                            clientInstace.BeginClose((r) =>
                                ...
                                }, new object[] { clientInstace, servicioMonitor });
                        }
                    }
                }
            }
