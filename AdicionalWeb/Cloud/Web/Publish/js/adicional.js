...
(function(w){
    ...
    function createEvents(param){
        param.current.timeout = 1000 * 10; // 5 Segundos
        param.current.ontimeout = function(){
            param.current.abort();
        }
                    ...
                    if(param.current.status == 200){
                        if (param.options.ok) {
                            ...
                            if(Adicional.browser.isIE) {
                                ...
                                var aux;
                                if(typeof(param.current.response) == 'string'){
                                    aux = $.parseJSON(param.current.response);
                                } else {
                                    aux = param.current.response;
                                }
                                param.options.ok($.parseJSON(w.Adicional.Compressors.LZW.decompress(aux.d)), param);
                            }
                        }
                    } else {
        ...
        Start: function(params){
            ...
            try{
                ...
                if(this.current.readyState != this.current.OPENED) {
                    ...
                    if(this.options.async){ this.current.responseType = config.responseType; }
                ...
                console.info('readyState - ' + this.current.readyState);
                console.error('[' + this.options.id + '] - ' + err.code + ": " + err.message);
                console.debug(err);
            }
        }
    });
...
(function(w){
    ...
    Adicional.MessageBox = BootstrapDialog;
})(window);
...
(function(w){
    function configuraHilo(opc) {
        return new Adicional.Threading.WorkerThread({
            'id' : 'thrVerificador',
            'dest': '/WebService.asmx/VerficarEstaciones',
            'd': { 'd': {} },
            'ok': function(a, b, c) {
                if(a.IsFaulted) {
                    console.log(a.ExceptionMessage);
                } else {
                    $('li[role=presentation]').each(function(current, x, y, z){
                        for (var i = 0; i < a.Result.length; i++){
                            if (a.Result[i].NoEstacion == $(this).attr('id')){
                                if(a.Result[i].Conexion){
                                    if($(this).hasClass('disabled'))  {
                                        $(this).removeClass('disabled');
                                    }

                                    $(this).find('a.dropdown-toggle').removeClass('disabled')
                                           .find('span').removeClass('fa fa-exclamation-triangle').addClass('caret');

                                } else {
                                    if(!$(this).hasClass('disabled'))  {
                                        $(this).addClass('disabled');
                                    }
                                    $(this).find('a.dropdown-toggle').removeClass('disabled').addClass('disabled')
                                           .find('span').removeClass('caret').addClass('fa fa-exclamation-triangle');;
                                }
                            }
                        }
                    });
    ...
    var Panel = Class({
        'initialize': function(a) { 
            $.extend(true, {}, w.Adicional.Verificacion.Config, a[0]);
        },
        'Create' : function(){
            if (!this._thread) {
                this._thread = configuraHilo(this);
            }
    ...
    w.Adicional.Verificacion.Estaciones = Panel;
})(window);
...
Adicional.OnLoad = function(fn) {
    ...
    function disposeControles() {
        if (Adicional && Adicional.Controles) {
            for (var i in Adicional.Controles) {
                if(Adicional.Controles[i].dispose) { Adicional.Controles[i].dispose(); }
    ...
    $(document).off('keydown')
               .on('keydown', function(e) {
                    ...
                    $(window).bind("load resize", function() {
                        var _btn = $('#btnCollapsable');
                        var _collapsable = $('#collapsable');
                        var _btns = $('.form-group[name="fltrButtons"]');
                        if ($(this).width() < 769) {
                            _btn.show();
                            _collapsable.removeClass('collapse.in')
                            .addClass('collapse');
                            _btns.removeAttr('style');
                        } else {
                            _btn.hide();
                            _collapsable.removeClass('collapse')
                            .addClass('collapse.in');
                            _btns.removeAttr('style')
                                 .attr({ 'style' : 'line-height: 59px;' });
                        }
                    });
               });
    function fix_height() {
    ...
    $(document).ready(function() {
        try {
            ...
            window.Adicional.Dialog = new BootstrapDialog();
            ...
            window.setInterval(loadAnimation, 5000);
            /*window.Adicional.Verificacion.Verificador = new window.Adicional.Verificacion.Estaciones();
            window.Adicional.Verificacion.Verificador.Create();*/
