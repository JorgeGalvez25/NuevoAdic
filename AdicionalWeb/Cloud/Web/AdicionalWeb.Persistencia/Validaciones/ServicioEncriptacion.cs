...
namespace AdicionalWeb.Persistencia.Validaciones
{
    public class ServicioEncriptacion
    {
        ...
        public string EncryptString(string plainMessage, string Key, string IV)
        {
            byte[] key = UTF8Encoding.UTF8.GetBytes(Key);
            byte[] iv = UTF8Encoding.UTF8.GetBytes(IV);
            return EncryptString(plainMessage, key, iv);
        }

        public string EncryptString(string plainMessage, byte[] Key, byte[] IV)
        {
        ...
        public string DecryptString(string encryptedMessage, string Key, string IV)
        {
            byte[] key = UTF8Encoding.UTF8.GetBytes(Key);
            byte[] iv = UTF8Encoding.UTF8.GetBytes(IV);
            return DecryptString(encryptedMessage, key, iv);
        }
         ...
         * @return	void
         */
        public void EncryptToFile(string plainMessage, string filename, byte[] Key, byte[] IV)
        {
            Array.Resize<byte>(ref Key, _keySize);
            Array.Resize<byte>(ref IV, _ivSize);
         ...
         */
        public string DecryptFromFile(string filename, byte[] Key, byte[] IV)
        {
            Array.Resize<byte>(ref Key, _keySize);
            Array.Resize<byte>(ref IV, _ivSize);
            // Crear un flujo para el archivo a generarse
