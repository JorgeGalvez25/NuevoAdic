...
namespace AdicionalWeb.Persistencia
{
    public class ManguerasPersistencia
    {
        ...
        {
            List<int> lstPosiciones = this.ObtenerListaPosiciones(est);
        ...
        private AdicionalWeb.Entidades.ListaDispensarios llenarListaTeam(Estacion est, List<int> listaPosiciones)
        {
            AdicionalWeb.Entidades.ListaDispensarios lstResult = new AdicionalWeb.Entidades.ListaDispensarios();
            int dispensario = 1;
            Historial mangueras = null;
            listaPosiciones.ForEach(p =>
                {
                    if ((p % 2) == 1)
                    {
                        mangueras = this.ObtenerPorPosicion(est, p).OrderBy(q => q.Manguera).FirstOrDefault();
                        lstResult.Add(new AdicionalWeb.Entidades.Dispensarios()
                            {
                                id = mangueras.Manguera,
                                estacion = mangueras.Id_Estacion,
                                dispensario = dispensario,
                                posicion = dispensario,
                                combustible = mangueras.Combustible,
                                nombre = "Cambiar...",//string.Format("Dispensario: {0:D2}", dispensario),
                                valor = decimal.ToDouble(mangueras.Porcentaje),
                                showTitleId = false
                            });
                        dispensario++;
                    }
