...
{
    public partial class viewBD : DevExpress.XtraEditors.XtraUserControl, IPage
    {
        public viewBD()
        {
            InitializeComponent();

            this.txtCarpeta.Text = Path.Combine(Constantes.TargetDir, Constantes.DataBaseFolder);

            this.Parent = new DevExpress.XtraWizard.WizardPage();
            this.Parent.Text = Constantes.Titulos.RutaBD;
        ...
        public void NextClick(object sender, DevExpress.XtraWizard.WizardCommandButtonClickEventArgs e)
        {
            try
            {
                ...
                WorkItem.Objetos<DirectoryInfo>.Add("ruta base datos", dir);
            }
            catch (Exception)
            {
