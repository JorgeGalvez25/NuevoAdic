...
namespace Servicios.Adicional.Sockets
{
    ...
    public class ServiciosSocket
    {
        ...
        //        StateObject state = new StateObject();
        //        state.Buffer = new byte[this.MaxBufferSize];
        //        state.WorkSocket = handler;
        ...
        //            try
        //            {
        //                handler.BeginReceive(state.Buffer, state.OffSet, state.Buffer.Length, SocketFlags.None, new AsyncCallback(receiveCallback), state);
        //            }
        //            catch (Exception e)
        //            {
        //                ////Si se presenta un problema de comunicación, se registra en bitácora y se cierra el socket.
        ...
        private byte[] request(byte[] buffer, int offSet)
        {
            ...
            try
            {
                ...
                peticion = Serializador.Deserializar<SolicitudAdicional>(buffer, ProtocoloSerializacion.Socket);
            ...
            try
            {
                ...
                //if (!ConfigMaster.Host.Log.SoloErrores)
            ...
            {
                buffer = Serializador.Serializar(respuesta, ProtocoloSerializacion.Socket);
